...
struct ContentView: View {
    ...
    @Environment(AppTerminationCoordinator.self) private var appTerminationCoordinator
    ...
    @State private var lastSavedSourceCode: String = ""
    ...
    @State private var windowCloseCoordinator = WindowCloseCoordinator()
    @State private var isCloseConfirmationPresented = false
    @State private var shouldBypassCloseConfirmation = false
    @State private var hasClaimedPendingRelaunchDocument = false
    @State private var editorWindowID = UUID()
    ...
    var body: some View {
            ...
            root
                .background(WindowReader { window in
                    if hostWindow !== window {
                        if let hostWindow {
                            appTerminationCoordinator.unregister(window: hostWindow)
                        }
                        hostWindow = window
        ...
        .sheet(isPresented: $isGoToLineSheetPresented) {
            VStack(alignment: .leading, spacing: 14) {
                ...
                TextField(String(), text: $selectedLineText)
    ...
    private func runSimulation() {
        ...
        do {
            ...
            simulationSession.sourceEditorWindowID = editorWindowID
    ...
    private func performSave() {
        if let fileURL = currentFileURL {
            _ = writeSource(to: fileURL)
            return
        }

        presentSavePanel(for: hostWindow)
    }
    ...
    @discardableResult
    private func writeSource(to fileURL: URL) -> Bool {
        let hasAccess = fileURL.startAccessingSecurityScopedResource()
        defer {
            if hasAccess {
                fileURL.stopAccessingSecurityScopedResource()
    ...
    private func installNotificationObservers() {
        ...
        notificationObservers.append(center.addObserver(forName: .simulaShowEditorWindow, object: nil, queue: .main) { _ in
            if let sourceEditorWindowID = simulationSession.sourceEditorWindowID,
               sourceEditorWindowID != editorWindowID {
                return
            }
            hostWindow?.makeKeyAndOrderFront(nil)
            NSApp.activate(ignoringOtherApps: true)
    ...
    private var hasUnsavedChanges: Bool {
        sourceCode != lastSavedSourceCode
    }
    ...
    private func configureWindow(_ window: NSWindow?) {
        ...
        windowCloseCoordinator.onShouldClose = { closingWindow in
            handleWindowShouldClose(closingWindow)
        }
        windowCloseCoordinator.onWillClose = { closingWindow in
            if hostWindow === closingWindow {
                hostWindow = nil
            }
            appTerminationCoordinator.unregister(window: closingWindow)
        }
        if window.delegate !== windowCloseCoordinator {
            window.delegate = windowCloseCoordinator
        }
        syncWindowDocumentState()
        restorePendingRelaunchDocumentIfNeeded()
    }

    private func restorePendingRelaunchDocumentIfNeeded() {
        guard !hasClaimedPendingRelaunchDocument else { return }
        hasClaimedPendingRelaunchDocument = true

        guard currentFileURL == nil, sourceCode.isEmpty else { return }
        guard let url = appTerminationCoordinator.claimNextPendingRelaunchDocument() else { return }

        loadSource(from: url)
    }

    private func syncWindowDocumentState() {
        guard let window = hostWindow else { return }
        window.isDocumentEdited = hasUnsavedChanges
        window.representedURL = currentFileURL
        window.title = currentDocumentTitle
        appTerminationCoordinator.register(
            window: window,
            hasUnsavedChanges: { hasUnsavedChanges },
            currentDocumentURL: { currentFileURL },
            requestTerminationDecision: { completion in
                requestTerminationDecision(for: window, completion: completion)
            }
        )
    }

    private func handleWindowShouldClose(_ window: NSWindow) -> Bool {
        if shouldBypassCloseConfirmation {
            return true
        }
        ...
        presentCloseConfirmation(for: window, shouldOpenReplacementWindow: true) { _ in }
        return false
    }

    private func requestTerminationDecision(for window: NSWindow, completion: @escaping (Bool) -> Void) {
        if !hasUnsavedChanges {
            completion(true)
            return
        }

        presentCloseConfirmation(for: window, shouldOpenReplacementWindow: false, completion: completion)
    }

    private func presentCloseConfirmation(
        for window: NSWindow,
        shouldOpenReplacementWindow: Bool,
        completion: @escaping (Bool) -> Void
    ) {
        isCloseConfirmationPresented = true

        let alert = NSAlert()
        alert.messageText = L10n.tr("alert.unsavedChanges.title", languageSelectionRaw: languageRawValue)
        alert.informativeText = ""
        alert.addButton(withTitle: L10n.tr("button.save", languageSelectionRaw: languageRawValue))
        alert.addButton(withTitle: L10n.tr("button.discard", languageSelectionRaw: languageRawValue))
        ...
        alert.beginSheetModal(for: window) { response in
            ...
            case .alertFirstButtonReturn:
                handleSaveAndClose(
                    window,
                    shouldOpenReplacementWindow: shouldOpenReplacementWindow,
    ...
    private func handleSaveAndClose(
        ...
        if let currentFileURL {
            guard writeSource(to: currentFileURL) else { return }
            if appTerminationCoordinator.isTerminationInProgress {
                appTerminationCoordinator.registerPendingRelaunchDocument(currentFileURL)
            }
            closeWindow(window, shouldOpenReplacementWindow: shouldOpenReplacementWindow) {
                completion(true)
            }
            return
        }
    ...
    private func discardChangesAndClose(
        ...
        if appTerminationCoordinator.isTerminationInProgress {
            appTerminationCoordinator.unregisterPendingRelaunchDocument(currentFileURL)
        }
        closeWindow(window, shouldOpenReplacementWindow: shouldOpenReplacementWindow) {
            completion(true)
        }
    ...
    private func closeWindow(
        ...
        DispatchQueue.main.async {
            appTerminationCoordinator.unregister(window: window)
            window.close()
            hostWindow = nil
            shouldBypassCloseConfirmation = false
            if shouldOpenReplacementWindow && !appTerminationCoordinator.isTerminationInProgress {
                openWindow(id: "main-window")
            }
            completion?()
        }
    ...
    private func presentSavePanel(
        for preferredWindow: NSWindow?,
        closeAfterSave: Bool = false,
        shouldOpenReplacementWindow: Bool = false,
        ...
        panel.prompt = L10n.tr("button.save", languageSelectionRaw: languageRawValue)
        panel.nameFieldStringValue = suggestedSaveFilename
        panel.canCreateDirectories = true
        panel.allowedContentTypes = [ASMSourceDocument.asmSourceType]
        panel.isExtensionHidden = false

        guard let window = preferredWindow ?? hostWindow ?? NSApp.keyWindow ?? NSApp.mainWindow ?? NSApp.windows.first else {
        ...
        panel.beginSheetModal(for: window) { response in
            ...
            guard response == .OK, let fileURL = panel.url else {
                if closeAfterSave && !shouldOpenReplacementWindow && appTerminationCoordinator.isTerminationInProgress {
                    closeWindow(window, shouldOpenReplacementWindow: false) {
                        onComplete?(true)
                    }
                    return
                }
            ...
            guard writeSource(to: fileURL) else {
                onComplete?(false)
                return
            }
            currentFileURL = fileURL
            if appTerminationCoordinator.isTerminationInProgress {
                appTerminationCoordinator.registerPendingRelaunchDocument(fileURL)
            }
...
private struct QuickGuideView: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                Text("\(guideText)\n\nⓘ \(L10n.tr("guide.partialLocalizationNote", languageSelectionRaw: languageRawValue))")
                    .font(.body)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(20)
                    .textSelection(.enabled)
    ...
    private var guideText: String {
        L10n.tr("guide.body", languageSelectionRaw: languageRawValue)
    }
}
...
private final class WindowCloseCoordinator: NSObject, NSWindowDelegate {
    var onShouldClose: (NSWindow) -> Bool = { _ in true }
    var onWillClose: (NSWindow) -> Void = { _ in }

    func windowShouldClose(_ sender: NSWindow) -> Bool {
        onShouldClose(sender)
    }
        ...
        VStack(spacing: 10) {
            HStack {
                ...
                Button(L10n.tr("sim.backToEditor", languageSelectionRaw: languageRawValue)) {
                    ...
                    NotificationCenter.default.post(name: .simulaShowEditorWindow, object: simulationSession.sourceEditorWindowID)
                    dismiss()
                }
                .buttonStyle(.glass)
                .tint(brandAccent)
            ...
            HStack(spacing: 10) {
                ...
                Text(L10n.tr("sim.breakpoint", languageSelectionRaw: languageRawValue))
                    .foregroundStyle(.secondary)
                TextField(String("0x00400000"), text: $breakpointText)
                    .textFieldStyle(.roundedBorder)
                    .font(.system(.subheadline, design: .monospaced))
                    .frame(width: 120)

                Text(verbatim: "\(L10n.tr("sim.cycle", languageSelectionRaw: languageRawValue)) \(max(simulationSession.currentCycleIndex + 1, 0))/\(simulationSession.cycleSnapshots.count)")
                    .font(.headline)
            }
...
private struct ConfigNumericStepper: View {
...
private struct MulticycleConfigSheet: View {
    ...
    private func stepperRow(_ title: String, value: Binding<Int>) -> some View {
        HStack {
            ...
            Spacer()
            ConfigNumericStepper(value: value, range: 1...99)
        }
    }
...
private struct ScoreboardConfigSheet: View {
    ...
    private func scoreRow(
        ...
        HStack {
            ...
            Spacer()
            ConfigNumericStepper(value: units, range: unitRange)
            ConfigNumericStepper(value: latency, range: latencyRange)
        }
    }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack {
                    Text(L10n.tr("cfg.int", languageSelectionRaw: languageRawValue))
                        .frame(width: 90, alignment: .leading)
                    Spacer()
                    ConfigNumericStepper(value: $draft.intUnits, range: 1...1)
                    ConfigNumericStepper(value: $draft.intLatency, range: 1...1)
                }
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text(title)
                    .frame(width: 90, alignment: .leading)
                Spacer()
                ConfigNumericStepper(value: row.units, range: 1...4)
                ConfigNumericStepper(value: row.latency, range: latencyRange(for: title))
            }
            Picker(L10n.tr("menu.segmented", languageSelectionRaw: languageRawValue), selection: row.segmented) {
