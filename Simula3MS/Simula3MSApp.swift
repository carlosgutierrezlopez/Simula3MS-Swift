...
struct Simula3MSApp: App {
    ...
    private var dataPathMenuSelection: Binding<DataPath> {
        Binding {
            settings.dataPath
        } set: { newValue in
            settings.dataPath = newValue

            if newValue == .multicycle {
                NotificationCenter.default.post(name: .simulaOpenMulticycleConfig, object: nil)
            } else if newValue != .segmented {
                settings.scheduling = .basic
            }
        }
    }

    private var segmentedMenuSelection: Binding<SchedulingModel> {
        Binding {
            settings.dataPath == .segmented ? settings.scheduling : .basic
        } set: { newValue in
            settings.dataPath = .segmented
            settings.scheduling = newValue
        ...
        .commands {
            CommandGroup(after: .toolbar) {
                ...
                Picker(L10n.tr("menu.language", languageSelectionRaw: languageRawValue), selection: $languageRawValue) {
                    ForEach(AppLanguageSelection.menuCases) { language in
                        HStack {
                            Text(language.localizedTitle(languageSelectionRaw: languageRawValue))
                            if language.showsPartialSupportWarning {
                                Spacer()
            ...
            CommandMenu(L10n.tr("menu.settings", languageSelectionRaw: languageRawValue)) {
                ...
                Menu(L10n.tr("menu.datapath", languageSelectionRaw: languageRawValue)) {
                    Picker(L10n.tr("menu.datapath", languageSelectionRaw: languageRawValue), selection: dataPathMenuSelection) {
                        Text(DataPath.monocycle.localizedTitle(languageSelectionRaw: languageRawValue)).tag(DataPath.monocycle)
                        Text(DataPath.multicycle.localizedTitle(languageSelectionRaw: languageRawValue)).tag(DataPath.multicycle)
                    }
                    .labelsHidden()

                    Menu(L10n.tr("menu.segmented", languageSelectionRaw: languageRawValue)) {
                        Picker(L10n.tr("menu.segmented", languageSelectionRaw: languageRawValue), selection: segmentedMenuSelection) {
                            Text(SchedulingModel.basic.localizedTitle(languageSelectionRaw: languageRawValue)).tag(SchedulingModel.basic)
                            Text(SchedulingModel.scoreboard.localizedTitle(languageSelectionRaw: languageRawValue)).tag(SchedulingModel.scoreboard)
                            Text(SchedulingModel.tomasulo.localizedTitle(languageSelectionRaw: languageRawValue)).tag(SchedulingModel.tomasulo)
                        }
                        .labelsHidden()
                    }
                }
...
final class AppTerminationCoordinator {
    ...
    func performPendingRelaunchIfNeeded() {
        ...
        let script = "sleep 0.8; /usr/bin/open -n '\(path)'"
...
private extension Simula3MSApp {
    ...
    func promptForRestart(selection: AppLanguageSelection, previousRawValue: String) {
        let alert = NSAlert()
        let restartMessage = L10n.tr("alert.restartRecommended.message", languageSelectionRaw: selection.rawValue)
        let partialMenuBarWarning = L10n.tr(
            "alert.restartRecommended.partialMenuBarWarning",
            languageSelectionRaw: selection.rawValue
        )

        alert.messageText = L10n.tr("alert.restartRecommended.title", languageSelectionRaw: selection.rawValue)
        alert.informativeText = selection.showsPartialSupportWarning
            ? "\(restartMessage)\n\n\(partialMenuBarWarning)"
            : restartMessage
    ...
    func relaunchApplication() {
        appTerminationCoordinator.queueApplicationRelaunch()
        NSApp.terminate(nil)
    }
